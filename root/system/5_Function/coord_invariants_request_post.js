// system/5_Function/coord_invariants_request_post.js

export function coord_invariants_request_post({ workflowContext, carrier }) {
    ...
    if (!hasPath && !hasReport) {
        return {
            phase: "invariants_request_post_pass_1",
            invariants_report: null,
    ...
    if (hasPath && hasReport && carrier.invariants_report.phase === "invariants_pass_1") {
        return {
            phase: "invariants_request_post_pass_1_checkpoint",
    ...
    if (hasPath && !hasReport) {
        return {
            phase: "invariants_request_post_pass_2",
            invariants_report: null,
            next_path: "coord_invariants"
        };
    }
    ...
    if (hasPath && hasReport && carrier.invariants_report.phase === "invariants_pass_2") {
        return {
            phase: "invariants_request_post_pass_2_checkpoint",
            invariants_report: carrier.invariants_report,
            next_path: "front_desk"
        };
    }
    ...
    return {
        ...
        invariants_report: null,
        next_path: null,
        error: "Postprocess invariants request reached an undefined state"
    };
}
